...
async function main() {
  ...
  // Seed sample users & funnel events over the past 14 days
  ...
  for (const userId of users) {
    // User signup timestamp between 14 days ago and now
    ...
    // Step 1: add_to_cart (70% conversion)
    if (Math.random() < 0.7) {
      ...
      // Step 2: checkout_started (45% conversion)
      if (Math.random() < 0.65) {
        ...
        // Step 3: payment_completed (30% total conversion)
        if (Math.random() < 0.7) {
  ...
  // Seed sample telemetry logs (services: auth-service, checkout-service, payment-gateway)
  const services = [
  ...
  for (let i = 0; i < 600; i++) {
    ...
    // Generate realistic log-normal distribution for latency percentiles
